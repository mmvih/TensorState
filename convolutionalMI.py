...
# Unit 1
conv_1 = keras.layers.Conv2D(3, 5, name='conv_1',**kwargs)(input_layer)
...
# Unit 2
conv_2 = keras.layers.Conv2D(9, 5, name='conv_2', **kwargs)(maxp_1)
conv_21 = keras.layers.Conv2D(9, 5, padding='same',name='conv_21', **kwargs)(conv_2)
norm_2 = keras.layers.BatchNormalization(epsilon=0.00001,momentum=0.9)(conv_21)
maxp_2 = keras.layers.MaxPool2D((2,2), name='maxp_2')(norm_2)

# Fully Connected
conv_3 = keras.layers.Conv2D(4, 4, name='conv_3', **kwargs)(maxp_2)
...
# modeltoload = 'seq' 
# modeltoload = 'lenet5'
# modeltoload = 'highest_aIQ'
modeltoload = 'twoconv_highest_aIQ'
tf.keras.models.save_model(model=model,filepath='./'+ modeltoload + '/model.h5')

""" Evaluate model efficiency """
# Attach StateCapture layers to the model
model = tf.keras.models.load_model(str(Path('.').joinpath(modeltoload).joinpath('model.h5')), compile=True)
# model_before = tf.keras.models.load_model(str(Path('.').joinpath(modeltoload).joinpath('model.h5')), compile=True)
efficiency_model = ts.build_efficiency_model(model=model,attach_to=['Conv2D'],method='both', storage_path='./both/')
...
#     print(layer.name, 100*layer.efficiency())
# for layer in efficiency_model_after.efficiency_layers:
#     print(layer.name, 100*layer.efficiency())
    # print(100*layer.efficiency())
...
for layer in model.layers:
    ...
    if layer.name == "input":
        ...
        if layer.padding == "same":
            test_images = np.pad(test_images, ((0,0),(4,4),(4,4),(0,0)), 'constant',constant_values=(0,0))
        ...
        MI = 0
        relative_entropy_i2o = 0
        relative_entropy_o2i = 0
        for ij in pxy_d.keys():
            i = ij[0]
            j = ij[1]
            add = pxy_d[ij]*math.log2(pxy_d[ij]/(px_d[i]*py_d[j]))
            rela_i2o_add = px_d[i]*(math.log2(px_d[i]/py_d[j]))
            rela_o2i_add = py_d[j]*(math.log2(py_d[j]/px_d[i]))
            MI = MI + add
            relative_entropy_i2o = relative_entropy_i2o + rela_i2o_add
            relative_entropy_o2i = relative_entropy_o2i + rela_o2i_add
            norm_MI = 2*MI/(shannons_input + shannons_output)
            # mutual_info.append(MI)
        mutual_info[layer.name] = MI
        relative_entropies[layer.name] = (relative_entropy_i2o, relative_entropy_o2i)
        normalized_mutual_info[layer.name] = norm_MI
            # print("MUTUAL INFORMATION: ", MI)
        # except Exception as e:
        #     mutual_info.append(MI)
        #     relative_entropies.append((relative_entropy_i2o, relative_entropy_o2i))
        #     continue
        ...
        test_image = test_images.flatten()
    else:
        print("NO FUNCTION FOR", layer.name)
        print("MUTUAL INFORMATION", mutual_info)
        print("NORMALIZED MUTUAL INFORMATION", normalized_mutual_info)
        print("RELATIVE ENTROPIES (INPUT TO OUTPUT), (OUTPUT TO INPUT)", relative_entropies)
